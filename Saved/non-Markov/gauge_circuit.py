...
# def run_grid_shots(grid, n_steps, *, shots=2048, save_every=1, verbose=True):
#     """Shot-basierter Lauf: pro Auslesezeit ein eigener Schaltkreis, echte Statistik.

#     Fuer jedes t wird ein frischer Schaltkreis mit t Anwendungen von U gebaut, die
#     Ancilla nach jedem Schritt in ihr eigenes klassisches Bit gemessen und
#     zurueckgesetzt, und der Lauf mit `shots` Shots ausgefuehrt.  p_total(t) ist
#     dann der *gemessene* Anteil der Shots, deren Protokoll aus lauter Nullen
#     besteht -- eine Binomialschaetzung mit dem ueblichen 1/sqrt(N)-Fehler, genau
#     wie auf echter Hardware.  Der Rueckgabewert enthaelt deshalb auch die
#     Fehlerbalken.

#     Unterschied zu `run_grid`: dort laeuft EIN Schaltkreis und alle Zeiten werden
#     per `save_statevector(conditional=True)` aus diesem einen Lauf gelesen.  Hier
#     wird pro Zeitpunkt neu praepariert und neu gemessen -- teurer, aber naeher an
#     dem, was ein Geraet tatsaechlich tut.

#     WAS HIER NICHT HARDWARE-TREU IST.  Die *Richtung* von y_t stammt weiterhin aus
#     Aers bedingtem Statevector.  Auf einem Geraet braeuchte man dafuer Tomographie
#     des Krylov-Registers oder einen Hadamard-Test je Komponente, denn
#     vec rho_S = R y ist ein LINEARES Funktional von y, waehrend Zaehlraten in der
#     Rechenbasis nur |y_i|^2 liefern.  Diese Kosten sind hier nicht enthalten --
#     es ist dieselbe Luecke, die auch der Transfertensor-Weg hat.  Echt gesampelt
#     ist die Normierung, und die ist der Teil, der die Post-Selection ausmacht.

#     Returns
#     -------
#     pops     : (len(times), d)  Populationen
#     pops_err : (len(times), d)  1-sigma-Statistikfehler aus der Binomialstreuung
#     info     : dict mit t_index, p_success, p_success_err, accepted, shots, qc
#     """
#     d, m, s, R = grid['d'], grid['m'], grid['s'], grid['R']
#     mp, U = grid['mp'], grid['U']
#     n_sys = int(np.log2(mp))
...
#     accepted = np.zeros(len(times), dtype=int)
#     qc = None

#     for k, t in enumerate(times):
#         if t == 0:                                   # der Startzustand ist bekannt
#             y_dir = np.zeros(m, complex); y_dir[0] = 1.0
#             prob[k], accepted[k] = 1.0, shots
#             pops[k] = np.real(np.diag(
#                 (R @ (y_dir * scale0)).reshape(d, d, order='F')))
#             pops_err[k] = 0.0
#             continue

#         q_sys = QuantumRegister(n_sys, 'krylov')
#         q_anc = QuantumRegister(1, 'anc')
#         c_anc = ClassicalRegister(t, 'rec')
#         qc = QuantumCircuit(q_sys, q_anc, c_anc)
#         for step in range(t):
#             qc.append(gate, q_sys[:] + q_anc[:])
...
#         if verbose:
#             print(f"  t = {t:4d}/{n_steps}", end='\r', flush=True)
#         result = sim.run(transpile(qc, sim), shots=shots).result()
#         counts = result.get_counts()

#         ok = sum(c for b, c in counts.items() if set(b) <= {'0', ' '})
#         if ok == 0:
#             if verbose:
#                 print(f"\n  Abbruch bei t={t}: 0 von {shots} Shots akzeptiert.")
#             break
#         accepted[k] = ok
#         prob[k] = ok / shots
#         # Binomialfehler.  Bei ok == shots kollabiert der Wald-Fehler auf exakt 0
#         # und unterschaetzt die Unsicherheit; dort nimmt man die Dreierregel,
#         # also die einseitige 95-%-Schranke 3/N.
#         prob_err[k] = (np.sqrt(prob[k] * (1 - prob[k]) / shots)
#                        if 0 < ok < shots else 3.0 / shots)

#         saved = result.data().get('statevector')
#         key = _accepted_key(saved) if saved else None
#         if key is None:
#             if verbose:
#                 print(f"\n  Abbruch bei t={t}: kein Statevector fuer den Erfolgszweig.")
#             break
#         y_dir = np.asarray(saved[key])[:m]
#         y_dir = y_dir / np.linalg.norm(y_dir)

#         rho = (R @ y_dir).reshape(d, d, order='F')
#         # Globale Phase ueber die Spurerhaltung fixieren (Tr rho_S = 1 > 0)
#         rho = rho * np.exp(-1j * np.angle(np.trace(rho)))
#         amp = scale0 * s ** t * np.sqrt(prob[k])
#         pops[k] = np.real(np.diag(rho)) * amp
#         # d(sqrt p)/sqrt p = dp/(2p)  ->  relativer Fehler der Amplitude
#         pops_err[k] = np.abs(pops[k]) * prob_err[k] / (2 * prob[k])

#     if verbose:
#         done = np.isfinite(prob)
#         print(f"\n  {int(done.sum())} Zeitpunkte, je ein eigener Schaltkreis, "
#               f"{shots} Shots; p_succ {prob[done][0]:.4f} -> {prob[done][-1]:.4f}",
#               flush=True)
#     info = dict(t_index=np.array(times), p_success=prob, p_success_err=prob_err,
#                 accepted=accepted, shots=shots, n_qubits=grid['n_qubits'], m=m, s=s)
#     return pops, pops_err, info
...
def run_grid_counts_rho(grid, n_steps, *, shots=8192, save_every=1, verbose=True):
    """Die VOLLE Dichtematrix aus Zaehlraten -- Diagonale UND Kohaerenzen.

    Warum die Kohaerenzen nicht so einfach gehen wie die Diagonale
    --------------------------------------------------------------
    Bei rho_jj half, dass r_j . y reell und >= 0 ist: aus |<chi_j|y>|^2 folgte die
    Wurzel eindeutig.  rho_ij mit i != j ist dagegen KOMPLEX, und ein
    Betragsquadrat verliert die Phase.  Man bekommt |rho_ij|, mehr nicht.

    Die Phase holt man per Interferenz mit einer Referenz zurueck.  Sei
    a = r_ij . y und b = r_kk . y eine Diagonale (physikalisch reell und >= 0).
    Dann ist

        |a + b |^2 = |a|^2 + |b|^2 + 2 Re(a b*)
        |a + i b|^2 = |a|^2 + |b|^2 + 2 Im(a b*)

    also   a = ( Re(a b*) + i Im(a b*) ) / |b|.   Drei Analysevektoren statt
    einem: r_ij, r_ij + r_kk und r_ij + i r_kk.  Die unbekannte globale Phase von
    y kuerzt sich in a b* heraus, es braucht also weiterhin keine Phaseneichung.

    Klassisch gegengeprueft: max |Formel - wahr| = 2.7e-14 fuer Re und Im.
    Die Referenz wird dabei auf c = |a|/|b| skaliert; das drueckt die
    Fehlerverstaerkung (|a|^2 + c^2|b|^2)/(2|a| c |b|) auf exakt 1, waehrend die
    naive Wahl "groesste Diagonale" bis zu 5.9 ergab.

    GEMESSEN, und das ist die eigentliche Aussage: mit 4096 Shots sind die
    Kohaerenzen auf diesem Modell UNBRAUCHBAR -- Re rho_12 kam als -0.50 statt 0
    heraus (t=0) und als -0.003 statt 0.310 (t=10), waehrend die Diagonale mit
    2.96e-02 in Ordnung war.  Der Faktor 6 aus der Referenzskalierung repariert
    das nicht; noetig waeren rund 1e6...1e7 Shots pro Schaltkreis.  Wer die
    Kohaerenzen braucht, nimmt `run_grid` und liest `info['rho_tr']` -- exakt
    (2.3e-07 gemessen), ohne Shot-Rauschen und mit einem statt 22 Schaltkreisen.

    Kosten und Grenzen
    ------------------
    d + 3*d(d-1)/2 Schaltkreise pro Auslesezeit -- fuer d=4 also 22 statt 4.  Und
    die Statistik wird SCHLECHTER als bei der Diagonale, denn Re und Im entstehen
    als Differenz dreier aehnlich grosser Messwerte: der relative Fehler auf
    Re(a b*) waechst wie (|a|^2+|b|^2)/(2|a||b|) mal dem Fehler der Einzelmessung.
    Zusammen mit den ohnehin kleinen Ueberlappen (siehe `counts_readout_cost`)
    ist das auf diesem Modell eine Demonstration, keine praktikable Ableseroute.
    Wer die Kohaerenzen wirklich braucht, nimmt `run_grid` und liest
    `info['rho_tr']` -- exakt und ohne Shot-Rauschen.

    Returns (rho, rho_err, info) mit rho spur-normiert, (len(times), d, d).
    """
    d, m, s, R = grid['d'], grid['m'], grid['s'], grid['R']
    mp, U = grid['mp'], grid['U']
    n_sys = int(np.log2(mp))
    ...
    def measure(v, t):
        """Ein Analysevektor v: dreht v^dag/||v|| auf |0...0> und misst.

        Liefert (|v.y|^2 geschaetzt, dessen 1-sigma-Fehler, akzeptierte Shots).
        """
        nv = np.linalg.norm(v)
        chi = np.zeros(mp, complex)
        chi[:m] = np.conj(v) / nv
        ...
        q = zero / acc
        dq = np.sqrt(q * (1 - q) / acc) if 0 < zero < acc else 3.0 / acc
        return q * nv ** 2, dq * nv ** 2, acc

    rows_d = [R[j + d * j, :] for j in range(d)]
    pairs = [(i, j) for i in range(d) for j in range(i + 1, d)]
    times = [0] + [t for t in range(1, n_steps + 1)
                   if t % save_every == 0 or t == n_steps]
    if verbose:
        counts_readout_cost(grid, times, shots=shots, verbose=True)
        print(f"  Kohaerenzen: {d + 3*len(pairs)} Schaltkreise pro Zeitpunkt "
              f"({d} Diagonale + 3 x {len(pairs)} Offdiagonale)", flush=True)

    rho = np.full((len(times), d, d), np.nan, complex)
    rho_err = np.full((len(times), d, d), np.nan)

    for k, t in enumerate(times):
        diag, diag_err = np.zeros(d), np.zeros(d)
        bad = False
        for j in range(d):
            a2, da2, acc = measure(rows_d[j], t)
            if acc == 0:
                bad = True
                break
            diag[j], diag_err[j] = max(a2, 0.0), da2
        if bad:
            break
        ref = int(np.argmax(diag))               # staerkste Diagonale als Phasenreferenz
        b2, b2_err = diag[ref], diag_err[ref]
        babs = np.sqrt(b2) if b2 > 0 else 0.0

        M = np.zeros((d, d), complex)
        E = np.zeros((d, d))
        for j in range(d):                        # rho_jj = sqrt(|a|^2), reell >= 0
            M[j, j] = np.sqrt(diag[j])
            E[j, j] = diag_err[j] / (2 * np.sqrt(diag[j])) if diag[j] > 0 else np.nan
        for (i, j) in pairs:
            rij = R[i + d * j, :]
            a2, da2, acc = measure(rij, t)
            if acc == 0:
                bad = True
                break
            # Referenz auf |a| skalieren: c = |a|/|b|.  Der relative Fehler von
            # Re(a b*) skaliert wie (|a|^2 + c^2|b|^2)/(2|a| c |b|), und das ist
            # bei c = |a|/|b| genau 1 -- das Optimum.  Mit der groessten Diagonale
            # als Referenz (c = 1) waere es bis zu 5.9 gewesen.
            aabs = np.sqrt(max(a2, 0.0))
            c = aabs / babs if babs > 0 and aabs > 0 else 1.0
            m2, dm2, _ = measure(rij + c * rows_d[ref], t)
            m3, dm3, _ = measure(rij + 1j * c * rows_d[ref], t)
            re_ab = (m2 - a2 - c * c * b2) / (2 * c)
            im_ab = (m3 - a2 - c * c * b2) / (2 * c)
            val = (re_ab + 1j * im_ab) / babs if babs > 0 else 0.0
            derr = (np.sqrt(dm2 ** 2 + da2 ** 2 + (c * c * b2_err) ** 2)
                    / (2 * c * babs)) if babs > 0 else np.nan
            M[i, j], M[j, i] = val, np.conj(val)
            E[i, j] = E[j, i] = derr
        if bad:
            break
        tr = np.real(np.trace(M))
        if tr > 0:
            rho[k] = M / tr                       # Spur-Normierung kuerzt lambda_t
            rho_err[k] = E / tr
        if verbose:
            print(f"  t={t:4d}  Spur(roh)={tr:.4f}  "
                  f"|rho_12|={abs(M[0,1]):.4f}", flush=True)

    info = dict(t_index=np.array(times), shots=shots, n_qubits=grid['n_qubits'],
                m=m, s=s, circuits_per_time=d + 3 * len(pairs), pairs=pairs)
