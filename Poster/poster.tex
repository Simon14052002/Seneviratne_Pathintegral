...
\begin{postercolumn}
  ...
  \begin{block}{Markovian Dynamics: Lindblad $\rightarrow$ Kraus}
    Lindblad equation - the most general Markovian master equation:
    \[ \frac{d}{dt}\rho_S = \underbrace{-\tfrac{i}{\hbar}[H,\rho_S]}_{\text{unitary}}
       + \underbrace{\sum_\alpha \gamma_\alpha \Big( L_\alpha \rho_S L^\dagger_\alpha
         - \tfrac12 \{ L^\dagger_\alpha L_\alpha, \rho_S \} \Big)}_{\text{dissipator} \rightarrow \text{non-unitary}} := \mathcal{L} \rho_S \]
    \why{$\gamma_\alpha$: transition rate $\rightarrow$ Fermi's golden rule.
    $L_\alpha$: Lindblad operator $\rightarrow$ jump operator.}

    \textbf{Kraus operators directly from it.} For a small step $dt$ the
    Ansatz $M_0=\mathbb{1}+L_0\,dt$, $M_\alpha=\sqrt{dt}\,L_\alpha$ and
    $\sum_k M^\dagger_k M_k=\mathbb{1}$ fix $L_0$, which gives
    \[ M_0 = \mathbb{1} + \Big(-\tfrac{i}{\hbar}H
             - \tfrac12\sum_\alpha \gamma_\alpha L^\dagger_\alpha L_\alpha\Big) dt,
       \qquad M_\alpha = \sqrt{\gamma_\alpha\,dt}\;L_\alpha \]
    \why{Exact only to $\mathcal{O}(dt)$ -- it forces small steps. The
         algorithm therefore takes the route below, which is exact for
         \emph{any} $t$.}

    \textbf{1. Vectorise} $\Rightarrow$ linear, integrable in closed form:
    \[ \mathrm{vec}(\rho_S(t)) = e^{\mathcal{L}t}\,\mathrm{vec}(\rho_S(0))
       \equiv \Lambda(t)\,\mathrm{vec}(\rho_S(0)) \]
  ...
  \begin{block}{Why the Circuit Looks the Way It Does}
    ...
    \figcap{Seven-site FMO: three system qubits carry $|\psi\rangle$, the
      fourth is the ancilla, framed by two Hadamard gates.}

    Propagating $|\psi\rangle\otimes|0\rangle$ through the circuit and using
    $\Sigma_++\Sigma_-=2\Sigma$:
    \fitline{H\,U_\Sigma\,V^\dagger H\,|\psi\rangle\otimes|0\rangle
    ...
    \textbf{The enhanced algorithm.} In the loop above the state is
    re-prepared classically at every time step. Instead, the state of the
    ...
    \begin{itemize}\tightlist
      ...
      \item the growth of the classical cost with $t$ disappears
    \end{itemize}
  \end{block}
  ...
  \begin{block}{Result: Markovian Dynamics}
    ...
    \figcap{Site populations of the seven-site FMO complex over
      $1000\,$fs. Lines: classical reference, dots: quantum simulation.}
  \end{block}%
\end{postercolumn}\hspace{\colsep}%
%=======================================================================
%%%%%%%%%%%%%%%%%%%%%%%%  S P A L T E   3  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{postercolumn}
  ...
  \begin{block}{Non-Markovian Dynamics: HEOM}
    ...
    \textbf{hierarchical equations of motion} drop it: the bath degrees of
  ...
  \begin{block}{The Non-Markovian Propagator on a Circuit}
    Following Dan \emph{et al.}~[5]: HEOM supplies a propagator $G(t)$ whose
    $m$-th column holds the populations for an initial excitation on site
    $m$:
    \[ |\psi(t)\rangle = G(t)\,|\psi(0)\rangle \]
    \fitline{G(t) = \left\{
      \begin{pmatrix}
    ...
    \why{Four-site case. Superscript: site of the initial excitation, hence
         $G(t_0)=\mathbb{1}$; $G$ is a \emph{list}, one matrix per step.}
    \[ G(t) = U\,\Sigma\,V^\dagger \]
    ...
    \[ H\,U_\Sigma V^\dagger H\,|\psi(0)\rangle\otimes|0\rangle
